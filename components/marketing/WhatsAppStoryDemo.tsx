  ...
  return (
    <div className="rounded-[2.5rem] bg-[#0f1814] p-5 text-white shadow-[0_30px_80px_rgba(23,23,20,0.18)] sm:p-8">
      <div className="flex items-center gap-3 border-b border-white/10 pb-5">
        <span className="grid size-11 place-items-center rounded-full bg-[#ff8b70] font-black text-black">D</span>
        <div>
          <p className="text-sm font-bold">Dharma</p>
          <p className="text-xs text-white/45">contacto conocido</p>
        </div>
        <span className="ml-auto rounded-full border border-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-white/45">
          {view === 'compose' ? 'Prepara' : 'Martina recibe'}
        </span>
      </div>

      <div className="mt-6 flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.14em] text-white/40" aria-hidden="true">
        <span className={view === 'compose' ? 'text-[#d9ee73]' : undefined}>1 · Dharma</span>
        ...
        {view === 'compose' ? (
          <div className="mt-7">
            ...
            <div className="mt-2 flex items-center justify-between gap-3 text-[10px] text-white/35">
              <span>Se abre en el WhatsApp de Dharma.</span>
            ...
            <div className="mt-6 rounded-3xl border border-dashed border-white/15 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-white/55">Texto, preview y link personal para Martina.</p>
            </div>
          </div>
        ) : (
          <div className="pt-7">
            <div className="ml-auto max-w-[92%] rounded-[1.5rem_1.5rem_0.35rem_1.5rem] bg-[#d9ee73] p-4 text-[#171714]">
              ...
              <div className="mt-4 overflow-hidden rounded-2xl bg-[#f0eee8] p-4">
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-black/45">Alista · Invitación personal</p>
            ...
            <div className="mt-5 flex items-center gap-2 text-xs text-white/45">
              ...
              Martina reconoce quién le escribe. Alista vive dentro del link.
            </div>
          </div>
        )}
      </div>
      ...
      <button
        type="button"
        onClick={() => setView((current) => (current === 'compose' ? 'received' : 'compose'))}
      ...
      >
        ...
        <ArrowRight className={`size-4 transition-transform ${view === 'received' ? 'rotate-180' : ''}`} aria-hidden="true" />
      </button>
    </div>
  )
}
