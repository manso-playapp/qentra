...
export default function AccessLoginForm({ nextPath }: AccessLoginFormProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // El intercambio del codigo lo hace /acceso/callback en el servidor, que
      // deja la sesion en cookies y respeta el destino original.
      const redirectTo = new URL('/acceso/callback', window.location.origin)
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 space-y-4">
      ...
      <div className="flex items-center gap-3 pt-1" aria-hidden="true">
        ...
        <span className="text-xs font-medium text-white/40">o</span>
        <span className="h-px flex-1 bg-white/15" />
      </div>
      ...
      <button
        ...
        disabled={loading}
        className="inline-flex min-h-13 w-full items-center justify-center gap-3 rounded-full border border-white/20 bg-white/5 px-6 text-sm font-bold text-white transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white disabled:cursor-not-allowed disabled:opacity-60"
      >
        <GoogleMark />
...
function GoogleMark() {
  return (
    <svg viewBox="0 0 18 18" className="size-[18px]" aria-hidden="true" focusable="false">
      ...
      <path fill="#EA4335" d="M9 3.58c1.32 0 2.5.46 3.44 1.35l2.58-2.58C13.46.9 11.43 0 9 0A9 9 0 0 0 .93 4.95l3.04 2.33C4.68 5.16 6.66 3.58 9 3.58Z" />
    </svg>
  )
}
